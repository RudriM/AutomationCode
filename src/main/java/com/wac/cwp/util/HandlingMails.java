...
public class HandlingMails extends TestBase {
	...
	public enum EmailFolder {
	  ...
	  public HandlingMails(String username, String password, EmailFolder emailFolder) throws MessagingException {
		    ...
		    try {
		      props.load(new FileInputStream(new File("C:\\Workspace\\CustomerWebPortal\\src\\main\\resources\\email.properties")));
		    } catch(Exception e) {
		      e.printStackTrace();
		      System.exit(-1);
		    }
	...
	//************* GET EMAIL PROPERTIES *******************
	  ...
	  public String getCode() throws Exception {
		  Message messages[] = getMessagesBySubject("World Acceptance Corporation account email verification code", true, 5);
		  int numOfMsgs = messages.length;
		  log.info("Number of  unread messages with subject found is " +messages.length);
		  numOfMsgs = numOfMsgs- 1;
		  Message email = messages[numOfMsgs];
		  BufferedReader reader = new BufferedReader(new InputStreamReader(email.getInputStream()));
		  
		  String line;
		  String prefix = "Your code is:";

		  while ((line = reader.readLine()) != null) {
		     if(line.contains(prefix)) {
				  String code = line.substring(line.indexOf(":") + 2, line.indexOf(":") + 8);
				  log.info(code);
				  return code;
		      }
		   }
